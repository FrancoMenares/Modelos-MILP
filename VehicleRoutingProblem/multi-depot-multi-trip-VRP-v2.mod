...
param habilitado{D,K} binary default 0		; # 1 SI EL DEPOSITO ESTA HABILITADO PARA EL VEHICULO COMIENCE SUS OPERACIONES EN EL (DEPOSITO INICIAL). 0 SI NO
param compatible{D,K} binary default 0		; # 1 SI EL VEHICULO PUEDE VISITAR EL DEPOSITO. 0 SI NO
...
r1 {d in D, k in K, v in V: v=1} : sum {j in C: (d,j) in A} x[d,j,k,v] <= habilitado[d,k]	; # LOS VEHICULOS SOLO PUEDEN INICIAR OPERACIONES DESDE UN DEPOSITO HABILITADO

r3 {d in D, k in K, v in V: v!=1} : sum {j in C: (d,j) in A} x[d,j,k,v] <= compatible[d,k]	; # LOS VEHICULOS SOLO PUEDEN SALIR DE UN DEPOSITO COMPATIBLE
r4 {d in D, k in K, v in V} : sum {i in C: (i,d) in A} x[i,d,k,v] <= compatible[d,k]	; # LOS VEHICULOS SOLO PUEDEN LLEGAR A UN DEPOSITO COMPATIBLE
