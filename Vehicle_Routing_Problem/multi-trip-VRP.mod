...
set K := {1, 2}		; # CONJUNTO DE VEHICULOS
...
param capacidad  := 2				; # CAPACIDAD DE LOS VEHICULOS 
...
r3 {j in C} : sum{i in N, k in K, v in V: (i,j) in A} x[i,j,k,v] = 1 ; # SOLO SE PUEDE INGRESAR A UN CLIENTE UNA VEZ
r4 {i in C} : sum{j in N, k in K, v in V: (i,j) in A} x[i,j,k,v] = 1 ; # SOLO SE PUEDE SALIR DE UN CLIENTE UNA VEZ

r5 {m in C, k in K, v in V} : sum {i in N: (i,m) in A} x[i,m,k,v] = sum {j in N: (m,j) in A} x[m,j,k,v]	; # LOS CLIENTES SON VISITADOS POR UN UNICO VEHICULO

r6 {(i,j) in A, k in K, v in V: j not in D} : c[j,k,v] >= c[i,k,v] + costo[i,j] - B*(1 - x[i,j,k,v])	; # ELIMINA SUBTOURS

r7 {k in K, v in V} : sum {(i,j) in A : j in C} demanda[j]*x[i,j,k,v] <= capacidad	; # NO SE DEBE SUPERAR LA CAPACIDAD DE LOS VEHICULOS

r8 {d in D, k in K, v in V: v > 1} : sum {j in C: (d,j) in A} x[d,j,k,v] <= sum {i in C: (i,d) in A} x[i,d,k,v-1]	; # SECUENCIALIDAD DE LAS VUELTAS